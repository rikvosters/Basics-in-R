...
tail(sort(table(dog$BREED[dog$OWNER_SEX == "w" & dog$OWNER_AGE == "81-90"])),1)
...
for (i in 1:50){  # set counter i
  print(7 * i)         # show result on screen
}
for (i in 1:50){  # set counter i
  cat(7 * i, sep = " ")         # show result on screen
}

for (i in 1:50){  # set counter i
  cat("7 times", i,"equals", 7 * i, "\n", sep = " ")         # show result on screen
}
...
# wide to long
dth %>% 
  pivot_longer(cols = `1900`:`2013`, 
               names_to = "Year", 
               values_to = "AgeAdjustedDeathRate") -> dth
